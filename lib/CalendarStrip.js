...
class DateItem extends PureComponent {
  render() {
    const {
      highlight,
      highlightColor = '#6E99F1',
      item,
      marked,
      onItemPress,
      todayHighlightColor = '#6E99F1',
      showLunar,
    } = this.props;
    const solar = format(item, 'D');
    const today = isSameDay(item, TODAY)
    ...
    let circleBackgroundColor = normalBgColor;
    let dateNumberColor = normalTextColor;
    if (highlight) {
        circleBackgroundColor = highlightColor;
        dateNumberColor = hightlightTextColor;
        if (today) {
            circleBackgroundColor = todayHighlightColor;
        }
    } else if (today) {
        dateNumberColor = todayHighlightColor;
    }
...
class CalendarStrip extends Component {
  ...
  loadPreviousTwoWeek(originalDates) {
    console.tron.log('originalDates', originalDates);
    const originalFirstDate = originalDates[0];
    const originalLastDate = originalDates[originalDates.length-1];
